...
public class JanelaRetornar { //inicio da classe JanelaRetornar
	...
	private void initialize(Estacionamento estacionamento) {
		...
		lblCPF.setBounds(58, 42, 56, 16);
		frmRetornar.getContentPane().add(lblCPF); 
		...
		btnRetornarCarro.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				long CPF; int quiloAtual;
				
				CPF = Long.parseLong(textPlaca.getText());
				quiloAtual = Integer.parseInt(textQuilo.getText());
				
				double montante = estacionamento.devolverCarro(CPF, quiloAtual); //invoca metodo de Estacionamento para devolucao de carros alugados
