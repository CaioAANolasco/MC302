...
public class JanelaAlugar { //inicio da classe JanelaAlugar
	...
	private void initialize(Estacionamento estacionamento) {
		...
		frmAlugar.setBounds(100, 100, 418, 804);
		...
		btnAlugarCarro.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String nome, data; long CPF; int multas, id, diarias; boolean seguro, aluguelSucesso;
				...
				aluguelSucesso = estacionamento.alugarCarro(novoAluguel, id); //chama metodo de Estacionamento
				if(aluguelSucesso) {
		...
		btnAlugarCarro.setBounds(51, 509, 302, 108);
		...
		btnCancelar.setBounds(51, 629, 302, 115);
		frmAlugar.getContentPane().add(btnCancelar); //botao para cancelar operacao
		
		JLabel lblId = new JLabel("ID Desejado:");
		lblId.setFont(new Font("Tahoma", Font.PLAIN, 16));
		lblId.setBounds(37, 421, 123, 16);
		frmAlugar.getContentPane().add(lblId);
		
		textID = new JTextField();
		textID.setBounds(125, 450, 153, 22);
		frmAlugar.getContentPane().add(textID);
