	...
	private void initialize(Estacionamento estacionamento) {
		...
		btnProcurar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String placa = textPlaca.getText(), saida = ""; char linha;
				
				ClienteGaragem clienteProcurado = estacionamento.buscaClienteGaragem(placa);  //procura o cliente pelo metodo de Estacionamento
				linha = (char)(clienteProcurado.getCarro().getLocalizacao()[0] + 65);
				saida = saida + "O carro esta estacionado no andar " + clienteProcurado.getCarro().getAndarLocalizado() + ", linha " +
				linha + ", coluna " + clienteProcurado.getCarro().getLocalizacao()[1];
				JOptionPane.showMessageDialog(frmProcurar, saida);
				frmProcurar.dispose();
